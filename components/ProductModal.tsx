...
import UpstockMessageModal from './UpstockMessageModal';
...
const ProductModal: React.FC<ProductModalProps> = ({ isOpen, onClose, productToEdit }) => {
  ...
  const [upstockUrl, setUpstockUrl] = useState('');
  ...
  // --- Upstock Logic ---
  const prepareAndShowUpstockMessage = () => {
    const supplierWhatsapp = localStorage.getItem('dukan-supplier-whatsapp');
    const supplierName = localStorage.getItem('dukan-supplier-name') || 'Supplier';
    const storeName = localStorage.getItem('dukan-store-name');
    
    if (!storeName || !supplierWhatsapp) {
        alert("Something went wrong. Store Name or Supplier details are missing.");
        return;
    }

    const supplierFirstName = supplierName.split(' ')[0];
    const lowStockList = lowStockItems.map(
        (item, index) => `${index + 1}. ${t(item.name)} - ${item.stock} ${item.stockUnit}`
    ).join('\n');
    const message = `Namaste ${supplierFirstName} ji, yeh stocks bhej do:\n${lowStockList}\n\nStore: ${storeName}\nDhanyavaad.`;
    const encodedMessage = encodeURIComponent(message);
    const whatsappUrl = `https://wa.me/${supplierWhatsapp}?text=${encodedMessage}`;

    setUpstockMessage(message);
    setUpstockUrl(whatsappUrl);
    setIsUpstockMessageModalOpen(true);
  };
  
  const proceedWithUpstock = () => {
    const supplierWhatsapp = localStorage.getItem('dukan-supplier-whatsapp');
    if (!supplierWhatsapp) {
        setIsSupplierModalOpen(true);
    } else {
        prepareAndShowUpstockMessage();
    }
  };
  ...
  const handleUpstockClick = () => {
    ...
    if (!localStorage.getItem('dukan-store-name')) {
